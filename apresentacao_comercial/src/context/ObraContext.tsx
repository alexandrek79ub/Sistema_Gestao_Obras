...
export function ObraProvider({ children }: { children: ReactNode }) {
  const [obraAtiva, setObraAtiva] = useState<string>('OBRA');
  const [listaObras, setListaObras] = useState<string[]>(['OBRA']);
  ...
  useEffect(() => {
    fetch('/api/obras')
      ...
      .catch(err => console.error('Erro ao buscar lista de obras:', err));
  }, []);
