...
- 📅 **[Planejamento & Linha de Base](file:///c:/Users/Alexandre/Workspace/A11_SISTEMA_DE_GESTAO_OBRAS/skills/gestao/SKILL_GESTAO_01_PLANEJAMENTO.md)** | **[Cronograma & Reprogramação](file:///c:/Users/Alexandre/Workspace/A11_SISTEMA_DE_GESTAO_OBRAS/skills/gestao/SKILL_GESTAO_16_CRONOGRAMA_E_REPROGRAMACAO.md)**: Prazos, sequenciamento, cálculo determinístico de Caminho Crítico (CPM) via `scripts/calculadoras/calcular_cpm.py` e geração de Linha de Base (Baseline 01) multi-formato (CSV, Excel com Curva S gráfica, MS Project XML e Dashboard HTML via `scripts/gerar_cronograma_tmult.py`).
...
- 🛡️ **[Segurança (SST) & RH](file:///c:/Users/Alexandre/Workspace/A11_SISTEMA_DE_GESTAO_OBRAS/skills/gestao/SKILL_GESTAO_04_SEGURANCA.md)** | **[Vencimento Documental](file:///c:/Users/Alexandre/Workspace/A11_SISTEMA_DE_GESTAO_OBRAS/skills/gestao/SKILL_GESTAO_17_VENCIMENTO_DOCUMENTAL_SEGURANCA.md)**: EPI, EPC, NR-18, riscos, Histograma de Mão de Obra (Headcount/HH) e Organograma Funcional com Matriz RACI (`06_SST_E_RH/`).
...
- 📐 **Motores de Orçamento, Planejamento e Dossiê Contratual**: Execução via `python scripts/gerador_orcamento_mestre.py` (Motor Híbrido Cérebro/CPU v2.0 com AST) para resolver expressões matemáticas e gerar CSV/MD com trilha auditável, automação de precificação e fechamento Turnkey via `python scripts/precificar_obra_tmult.py`, consulta rápida à base oficial SINAPI SP via `python scripts/consultar_sinapi.py`, `scripts/gerar_cronograma_tmult.py` (gerador do cronograma físico-financeiro, Curva S em Excel com gráfico e MS Project XML), `scripts/gerar_dossie_contratacao_tmult.py` (gerador dos histogramas de mão de obra/equipamentos e Curvas ABC duplas em CSV/XLSX), `scripts/gerar_fluxo_caixa_tmult.py` (gerador do fluxo de caixa analítico, curva de desembolso, gráficos Excel nativos e análise de capital de giro em 3 cenários) e `scripts/gerar_organograma_visual.py` (gerador do organograma em PNG alta resolução e SVG vetorial), além de `extrair_carimbos.py` / `gerar_lista_desenhos.py` para catalogar pranchas PDF.
...
- **MAPA DE BUSCA (DOSSIÊ DA OBRA):** Se o usuário fizer perguntas sobre Orçamento, Custos, Contratos ou Prazos de uma obra específica, você DEVE procurar os arquivos fonte (Baseline) nas pastas correspondentes do Dossiê de Obra em `/projetos/[NOME_DA_OBRA]/`. A árvore completa é:
  ...
  - `02_ORCAMENTO_BASE_E_CONTRATOS`: Orçamento consolidado (158 itens), Curvas ABC (Serviços e Insumos), Proposta Comercial Turnkey e Proposta Técnica/Memorial Metodológico.
  ...
  - `05_SUPRIMENTOS_E_FINANCEIRO`: Fluxo de caixa analítico (CSV/XLSX/MD), Curva de Desembolso, Capital de Giro em 3 cenários, conciliação 3 pontas, notas fiscais e relatórios de medição.
