...
def gerar_relatorio_markdown(output_dir, config):
    sigla = config.get("sigla_obra", "TMULT")
    nome = config.get("nome_obra", "TMULT — Edifício Administrativo")
    prazo = int(config.get("prazo_meses", 6))
    md_path = os.path.join(output_dir, f"RELATORIO_AVANCO_FISICO_{sigla}.md")

    template_path = os.path.join(os.path.dirname(__file__), "templates", "avanco", "template_relatorio_avanco_fisico.md")
    if os.path.exists(template_path):
        with open(template_path, "r", encoding="utf-8") as f:
            conteudo = f.read()
        conteudo = (
            conteudo
            .replace("{{NOME_OBRA_UPPER}}", nome.upper())
            .replace("{{SIGLA_OBRA}}", sigla)
            .replace("{{PRAZO_MESES}}", str(prazo))
        )
    else:
        conteudo = f"# 📈 RELATÓRIO EXECUTIVO DE AVANÇO FÍSICO E CURVA S\n## {nome.upper()} ({sigla})\n\n> Linha de Base 01 ({prazo} Meses)\n"
