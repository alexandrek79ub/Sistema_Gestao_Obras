...
- 📐 **Motor de Orçamento, SINAPI e Automações de Engenharia**: Execução via `python scripts/gerador_orcamento_mestre.py` (Motor Híbrido Cérebro/CPU v2.0 com AST) para resolver expressões matemáticas e gerar CSV/MD com trilha auditável, automação de precificação e fechamento Turnkey via `python scripts/precificar_obra_tmult.py`, consulta rápida à base oficial SINAPI SP via `python scripts/consultar_sinapi.py`, e `extrair_carimbos.py` / `gerar_lista_desenhos.py` para catalogar pranchas PDF.
