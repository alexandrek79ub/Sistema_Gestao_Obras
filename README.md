...
- 📑 **Repositório de Templates ([`scripts/templates/`](file:///c:/Users/Alexandre/Workspace/A11_SISTEMA_DE_GESTAO_OBRAS/scripts/templates/)):** Desacopla laudos técnicos, termos de entrega, RDOs e manuais dos scripts Python, mantendo os motores enxutos e focados exclusivamente na lógica de cálculo.
- 📊 **Catálogos de Engenharia ([`apoio/`](file:///c:/Users/Alexandre/Workspace/A11_SISTEMA_DE_GESTAO_OBRAS/apoio/)):** Tabelas de referência em JSON auditáveis (`catalogo_funcoes.json`, `mapa_lotes_cpm.json`, `catalogo_lotes_takt.json`, `catalogo_fvs.json`) editáveis sem alterar o código dos motores.

> **Status da Modernização v2.0 Lean:** 10 scripts centrais já refatorados com mais de **4.400 linhas de código eliminadas**, conformidade 100% (6/6 eixos verdes na auditoria multi-eixo) e máxima eficiência em tokens de contexto.

| # | Script | Padrão | Função Principal | Saídas Geradas |
| :---: | :--- | :---: | :--- | :--- |
...
| **03** | `gerar_cronograma.py` | **v2.0 Lean** | Cronograma Físico-Financeiro, Curva S e CPM | `CRONOGRAMA.xlsx`, XML MS Project, HTML |
| **04** | `gerar_programacao_curto_prazo_takt.py` | **v2.0 Lean** | Esteira de Produção Lean Takt (WWP 52 Lotes) via catálogo | `PROGRAMACAO_CURTO_PRAZO_[OBRA].csv` |
| **05** | `sincronizar_esteira_e_lob.py` | **v2.0 Lean** | Sincronizador Bidirecional Esteira Takt $\leftrightarrow$ Linha de Balanço | `LINHA_DE_BALANCO.csv`, `RELATORIO_SOBREPOSICAO_LOB.json` |
...
| **07** | `gerar_histograma_sincronizado.py` | **v2.0 Lean** | Motor Universal de Sincronização do Histograma de Mão de Obra (Headcount & HH) | `dados_histograma_mo.json`, `HISTOGRAMA_MAO_DE_OBRA.csv/xlsx` |
...
| **09** | `reprogramar_cronograma.py` | **v2.0 Lean** | Reprogramação de Atividades e Linha de Balanço com Crashing via RUP | Versões de replanejamento e auto-sincronia de curto prazo |
...
| **12** | `gerar_fluxo_caixa.py` | **v2.0 Lean** | Projeção financeira em 3 cenários e Capital de Giro | `FLUXO_DE_CAIXA.xlsx/csv/md` |
...
| **15** | `gerar_planilha_medicao.py` | **v2.0 Lean** | Planilha Master evolutiva de 12 quinzenas | `PLANILHA_MEDICAO_QUINZENAL.xlsx` (ret. 5%) |
| **16** | `gerar_dossie_contratacao.py` | — | Dossiê de contratação e histogramas de recursos | Histogramas de MO (HH) e Equipamentos |
| **17** | `gerar_rdo.py` | **v2.0 Lean** | Sistema diário de RDO e painel de produção | `PAINEL_RDOS_OBRA.xlsx` e `RDOS/RDO-XXX.md` |
| **18** | `gerar_fvs_bloqueantes.py` | **v2.0 Lean** | Caderno de 8 FVSs com bloqueio de medições via catálogo | `CADERNO_FVS_BLOQUEANTES.md` e planilha |
...
| **21** | `gerar_estrutura_databook.py` | **v2.0 Lean** | Estruturação e controle das 5 pastas de closeout | Acervo permanente em `07_DATABOOK_E_ASBUILT/` |
