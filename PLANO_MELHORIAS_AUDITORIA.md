...
| AUD-010 | P1 | Bug confirmado | RDO pode ser emitido com valores presumidos, numeração incorreta e assinatura sem evidência | IMPLEMENTADO |
...
- **Status:** `IMPLEMENTADO` (conclusão da Fase 3).
- **Implementação da Fase 3:** Módulo canônico `scripts/common/qualidade_fvs.py` implementado com validação técnica estrita (código oficial FVS-01 a FVS-08 conforme catálogo, 100% dos itens do checklist normativo atendidos, medição quantitativa de tolerância em campo obrigatória, identificação explícita do responsável técnico e evidência verificável de assinatura digital com hash auditável). Removidos todos os defaults decisórios em `/campo` e no validador `campoStaging.ts`. O desbloqueio de medição ocorre estritamente para o status `APROVADA` sob lock em `REGISTRO_INSPECOES_CAMPO.md` e na matriz de bloqueio, permanecendo `⛔ Bloqueada` para qualquer pendência, rascunho ou reprovação.
...
- **Natureza:** bug confirmado.
- **Status:** `IMPLEMENTADO`.
- **Implementação da Fase 3:** `scripts/processar_coleta_campo.py` atualizado para iniciar a numeração em 001 (`max_num = 0` na ausência de diários anteriores), validar estritamente todos os campos obrigatórios de campo (data, ef_prop, ef_terc, eap, responsavel) sem inventar defaults factuais, calcular HH deterministicamente e emitir documento como `RASCUNHO / PENDENTE DE ASSINATURA` caso não haja evidência de assinatura, ou `OFICIAL` com hash sha256 auditável quando assinada. O template `_TEMPLATE_OBRA_NOVA` foi purgado de diários operacionais e sua planilha/fila resetadas para estado limpo. `scripts/gerar_rdo.py` atualizado para não injetar pilotos em novas obras.
...
| 14/09/2026 | Fase 5 / AUD-013 — correção de regressões | IMPLEMENTADO → IMPLEMENTADO | Codex / Tech Lead | A comitar | `python -m unittest discover -s scripts/tests` (25/25), `npm test` (22/22), `tsc --noEmit`, `npm run lint` (0 erros; 28 avisos preexistentes) e `npm run build` aprovados | Restaurada a paridade com `141402c` no serviço extraído: dependências LOB/takt sem novas arestas implícitas, erros de parsing do curto prazo propagados e campos ausentes sem novos defaults. Testes de regressão executam o serviço isolado com dados mínimos, sem usar arquivos de obra. |
